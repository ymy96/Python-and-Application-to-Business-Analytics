...
SUIT_LIST = ("H", "S", "D", "C")       # Heart Spade, diamond, cube
...
class Deck(set):
    ...
    def __init__(self,Cards_Dealt):
        ...
        for rank, suit in itertools.product(RANK_LIST, SUIT_LIST):
            ...
            #print('card:',Card(rank,suit),type(Card(rank,suit)))
    ...
    def get_card(self):
        ...
        if len(self.Cards_Dealt) ==2:
            for x in range(2):
                for card in self:
                    if str(self.Cards_Dealt[x])==str(card):
                        self.remove(card)
                        break

                #print('dealtcardis:',card,type(card))

                #self.remove(card)
                #print('the left cards in deck after delete dealtcards:',len(self))
                #print('the type of dealtcards:',type(self.Cards_Dealt))
            self.Cards_Dealt=[]
    ...
    def get_hand_dealt(self, number_of_cards_dealt=2):
        """
        Retrieve a poker hand which consists of five cards.
        :param number_of_cards: number of cards to retrieve.
        :return:
        """
        if number_of_cards_dealt == 2:
            dealtCards= [self.get_card() for x in range(number_of_cards_dealt)]
            return dealtCards
        else:
            raise NotImplementedError

    def get_hand(self, number_of_cards=3):
        """
        Retrieve a poker hand which consists of five cards.
        :param number_of_cards: number of cards to retrieve.
        :return:
        ...
        if number_of_cards == 3:
            TheRestCards = [self.get_card() for x in range(number_of_cards)]
            cards5 =DealtCards+TheRestCards
            return PokerHand(cards5)
        else:
            raise NotImplementedError
...
DealtCards = Deck(Cards_Dealt=a).get_hand_dealt()
print('Your Dealt Cards are:',DealtCards,type(DealtCards))
for i in range(number_of_hands):
    hand_rank, details = Deck(Cards_Dealt=DealtCards).get_hand().get_hand_rank()
